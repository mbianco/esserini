function [x]=myplotstat(filename, multi)
statdata=fscanfMat( filename );
figure(1+3*multi);mtlb_hold( 'off');
plot(statdata(:,1)); 
mtlb_hold( 'on'); 
plot(statdata(:,3),'k'); 
plot(statdata(:,5),'r');
...
figure(2+3*multi);mtlb_hold( 'off');
plot(statdata(:,3)+statdata(:,2)+statdata(:,4)+statdata(:,5)); 
mtlb_hold( 'on'); 
plot(statdata(:,8),'r')
figure(3+3*multi); mtlb_hold( 'off'); 
